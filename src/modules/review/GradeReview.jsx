            ...
            {!singleData.length ? <div style={rs.empty}>该时段暂无视频数据</div> : (
              <table style={rs.table}>
                <thead><tr>
                  ...
                  <th style={rs.thR}>视频数</th><th style={rs.thR}>占比</th>
                  <th style={rs.thR}>出单视频</th><th style={rs.thR}>视频出单率</th>
          ...
          <div style={{ ...glassStyle(14), overflow:"auto" }}>
            <table style={rs.table}>
              <thead><tr>
                ...
                <th style={rs.thR}>总视频</th><th style={rs.thR}>出单率</th><th style={rs.thR}>总出单</th>
                {activeGrades.map(g=><th key={g} style={{ ...rs.thR,color:GRADE_COLORS[g]||T.muted }}>{g}<br/><span style={{ fontSize:FONT.xs,fontWeight:400,color:T.hint }}>均单/视频</span></th>)}
              </tr></thead>
              <tbody>
                {allData.map(({ym:m,rows})=>{
                  const total=rows.reduce((s,r)=>s+r.videoCount,0);
                  const totalO=rows.reduce((s,r)=>s+r.orders,0);
                  const totalW=rows.reduce((s,r)=>s+r.withSales,0);
                  return (
                    <tr key={m}>
                      ...
                      <td style={rs.tdR}>{rs.pct(total?totalW/total:null)}</td>
                      <td style={{ ...rs.tdR,fontWeight:600 }}>{totalO||"—"}</td>
                      {activeGrades.map(g=>{ const row=rows.find(r=>r.grade===g); return <td key={g} style={{ ...rs.tdR,color:GRADE_COLORS[g]||T.hint,fontWeight:row?.avgOrder?600:400 }}>{row?.avgOrder?rs.dec(row.avgOrder):"—"}</td>; })}
                    </tr>
                  );
                })}
              </tbody>
