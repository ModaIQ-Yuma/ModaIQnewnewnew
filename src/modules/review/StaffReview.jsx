...
export default function StaffReview({ collabs, videos, invites, products, staff }) {
  ...
  return (
    <div>
      ...
      {products.map((p) => {
        const rows = calcStaffMetrics(collabs, videos, ym, p.id);
            ...
            <div style={{ ...glassStyle(14), overflow: "hidden" }}>
              <table style={rs.table}>
                <thead><tr>
                  ...
                  <th style={rs.thR}>履约数</th>
                  <th style={rs.thR}>履约率</th>
                  <th style={rs.thR}>出单达人数</th>
                  <th style={rs.thR}>达人出单率</th>
                  <th style={rs.thR}>拉新数</th>
                </tr></thead>
                <tbody>
                  {rows.map((r) => {
