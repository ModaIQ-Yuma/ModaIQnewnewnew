...
export default function BurstWall({ videos, products, burstThreshold, onThresholdChange }) {
  ...
  const byProduct = useMemo(() =>
    products.map(p=>({ product:p, bursts:calcBurstVideos(videos,ym,p.id,burstThreshold,vFrom,vTo) }))
      .filter(({bursts})=>bursts.length>0),
    [videos,products,ym,burstThreshold,vFrom,vTo]
  );
          ...
          <div key={product.id} style={{ marginBottom:20 }}>
            ...
            <div style={{ ...glassStyle(14),overflow:"hidden" }}>
              <table style={rs.table}>
                <thead><tr>
                  <th style={rs.th}>达人</th><th style={rs.th}>视频链接</th><th style={rs.th}>发布日期</th>
                  <th style={rs.thR}>出单数</th><th style={rs.thR}>播放量</th><th style={rs.thR}>点击量</th><th style={rs.thR}>CTR</th>
                </tr></thead>
                <tbody>
                  {bursts.map(v=>(
                    <tr key={v.id}>
                      <td style={{ ...rs.td,fontWeight:600 }}>{v.creator_handle||"-"}</td>
                      <td style={rs.td}>{v.url?<a href={v.url} target="_blank" rel="noreferrer" style={{ color:T.accent,fontWeight:600,textDecoration:"none" }}>▶ 观看</a>:<span style={{ color:T.hint }}>-</span>}</td>
                      <td style={{ ...rs.td,color:T.muted }}>{v.published_at?.slice(0,10)}</td>
                      <td style={{ ...rs.tdR,fontWeight:800,fontSize:FONT.x4l,color:T.success }}>{v.orders}</td>
