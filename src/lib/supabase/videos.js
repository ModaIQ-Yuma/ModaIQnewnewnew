...
import { sb, unwrap, selectIn } from "./client.js";
...
export async function fetchProductsBySkuIds(storeId, skuIds) {
  ...
  return selectIn(skuIds, (part) =>
    sb.from("products").select("id, sku_id, internal_name").eq("store_id", storeId).in("sku_id", part), "products");
}
...
export async function fetchCollabsByHandles(storeId, handles) {
  ...
  const [creators, aliasRows] = await Promise.all([
    selectIn(names, (part) => sb.from("creators").select("id, handle").eq("store_id", storeId).in("handle", part), "creators"),
    selectIn(names, (part) => sb.from("creator_aliases").select("creator_id, alias").eq("store_id", storeId).in("alias", part), "creator_aliases"),
  ]);
  const nameToId = {};
  ...
  const collabs = await selectIn(ids, (part) => sb.from("collaborations").select("id, creator_id, product_id, ship_date")
    .eq("store_id", storeId).in("creator_id", part), "collaborations");
  const byCreator = {};
...
export async function fetchExistingVideos(storeId, videoIds) {
  ...
  return selectIn(videoIds, (part) =>
    sb.from("video_records").select("id, video_id, gmv, orders, clicks, vv").eq("store_id", storeId).in("video_id", part), "video_records");
}
