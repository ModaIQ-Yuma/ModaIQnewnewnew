...
import { within } from "./rangeCalc.js";

const blank = () => ({ shipCount: 0, fulfillCount: 0, withSalesCount: 0, videoCount: 0, videoOrders: 0, burstCount: 0, inviteCount: 0 });
const finish = (r) => ({
  ...r,
  fulfillRate: safeDiv(r.fulfillCount, r.shipCount), saleRate: safeDiv(r.withSalesCount, r.fulfillCount),
  sampleSalesRatio: safeDiv(r.videoOrders, r.shipCount),
});

/**
 * @param p { collabs, videos, invites, staff, productId?, ship, video, burst? }
 *   寄样/履约/出单：寄样区间内的寄样，看其在视频区间内的视频
...
export function calcStaffOverview(p) {
  ...
  const collabs = pick(p.collabs), videos = pick(p.videos).filter((x) => within(x.published_at?.slice(0, 10), p.video));
  ...
  const vidsByCollab = new Map();
  for (const x of videos) {
    if (!x.collaboration_id || !staffOfCollab.has(x.collaboration_id)) continue;
    vidsByCollab.set(x.collaboration_id, [...(vidsByCollab.get(x.collaboration_id) || []), x]);
  ...
  for (const c of collabs.filter((x) => within(x.ship_date, p.ship))) {
    const r = row(c.staff_id || "unknown"), vids = vidsByCollab.get(c.id) || [];
    r.shipCount++;
    if (vids.length) { r.fulfillCount++; if (vids.some((v) => (v.orders || 0) >= 1)) r.withSalesCount++; }
  }
